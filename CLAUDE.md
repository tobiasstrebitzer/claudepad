...
**Usage Insights - delegation, parallelism & limits (D-96…D-99, built 2026-09-10):** four corrections/additions to the reporting surface, all validated against the owner's real 30-day vault. (1) **Pricing** - `DEFAULT_PRICING` was missing `claude-opus-5`/`fable-5-1`/`sonnet-5`, and an unpriced model silently costs $0, so a vault that is 93% Opus 5 reported **$524** instead of **~$4.9K**; the table now covers the current tiers, `rate()` takes a cache-read override (Fable 5.1 reads at $0.25/MTok), `canonicalModel` strips `[1m]`, and `CostBreakdown.unpricedShare` makes any future gap render as a visible `>$X` floor. (2) **Subagents** - `scanVault` never descended into `<project>/<sessionId>/subagents/`, hiding ~14% of tokens; delegated runs are now scanned and summed into every total but counted as `agentRuns`, never as `sessions` (new `byAgentType` split + **Delegation** panel). (3) **Parallelism** - `usage/concurrency.ts` measures time-weighted concurrent top-level sessions (5-min slots, 10-min bridge), surfaced as an "At once" card + **Sessions in parallel** panel. (4) **Rate limits** - `quotaLimits` rejections lift into `EventBase.limit`; the **Limit hits** panel is an honest event log (incidents, days, blocked duration). Cache bumped to `file-aggregates-v3`.

**Usage Insights - subscription rate limits (PRD-14, D-100, built 2026-09-11):** what your plan actually meters, as opposed to what the API would charge. Calibrated by regressing 9,571 measured `utilization` readings (from a `ccstatusbar`-style quota log) against real token spend: **cache reads are free to the meter** (only fresh input + output + cache writes count - ~74% of a heavy user's API-equivalent cost is invisible to the limit, which is why quota burn feels random), metered over a **5h tumbling window** anchored to first use and a **weekly window** resetting Sun 04:00Z, plus a **Fable-scoped weekly sub-limit**. Bundled Max 20x budgets ($75 / $528 / $118) are reverse-engineered from one account, stated as such, editable, and superseded by self-calibration the moment you import your own quota log. `usage/{quota,quotaLog,useQuotaLog}.ts` + `usage/QuotaPanel.tsx`; three quality gates (monotone filtering, max-gap, unexplained-rise) stop a partial import from producing confidently wrong budgets. **Dev-only for now** via `QUOTA_PANEL_ENABLED` (`usage/availability.ts`, same pattern as `REGISTRY_ENABLED`); `VITE_QUOTA_PANEL=true` re-enables it in a production build.

The per-phase **PRDs were removed once implemented** (git history retains them); only `docs/prd/prd-07-backend.md` (the vNext store addon) and the canonical `docs/prd/_context.md` survive. The crypto reference is now `packages/crypto` itself, with `test/conformance.test.ts` as the end-to-end anchor (the standalone `poc/` was retired, D-85). Docs under `docs/` use lower-case-dash filenames; root files (`README.md`, `CHANGELOG.md`, `CONTRIBUTING.md`, `SECURITY.md`, this file) keep the standard uppercase convention.
...
docs/
  ...
  store-provider-spec.md# the ZK blob-store axis of registry-spec.md (open spec; vNext)
  decisions.md          # ★ decision log (D-1…D-89) + open questions + vNext backlog
  threat-model.md       # public, plain-language threat model
  self-hosting.md       # how to serve the static bundle
  verify-zero-knowledge.md # reproducible ZK verification steps
  prd/
    _context.md         # ★ canonical shared facts (tech stack, tokens, security, schema)
    README.md           # pointer: the per-phase PRDs were implemented & removed (in git history)
    prd-07-backend.md   # the one unbuilt PRD: optional store/backend addon (vNext)
apps/
...
### PRD index (historical - files removed, retained in git history)
The per-phase PRDs that drove the build, by number, for orientation:
- **01** Design System · **02** Parser & Schema · **03** Viewer · **04** Ingest & Share Output · **05** Crypto Core & Recipient Wrapping · **06** Secret Detection & Tiered Reveal · **08** Playback · **09** Self-Hosting (Static) & Launch · **10** Identity, Trust & Device Keys · **11** Trustless Recipient Sharing · **12** Viewer Themes.
- **07** (Store Provider Spec & Reference Impl) is **vNext / deferred** - not v1, and is the only PRD still in-tree (`docs/prd/prd-07-backend.md`).

...
export const TaskListTurn = React.memo(({
  event,
  result,
  anchorId,
  highlighted
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex items-center gap-2 text-label uppercase tracking-[0.02em] text-muted-foreground">
          ...
          <li key={i} className="flex items-start gap-2 text-body-sm">
            ...
            <div className="min-w-0">
              <span
                className={cn(
                  'break-words',
                  item.status === 'completed' ? 'text-muted-foreground line-through' : 'text-text'
                )}
              >
                {item.label}
              </span>
              {item.detail && item.detail !== item.label && (
                <p className="mt-0.5 break-words text-body-sm text-muted-foreground">{item.detail}</p>
              )}
            </div>
          </li>
        ))}

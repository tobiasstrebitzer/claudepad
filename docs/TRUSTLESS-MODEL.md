...
The serialized blob (`cp-blob-…`) contains **no plaintext** transcript or secret — only ciphertext plus the sender's *public* card (test [4]). It's dropped anywhere: a public Slack channel, an email, a `.cpblob` file. Inert to everyone except the recipient.

...
- **[ Share… ]** routes into the **PRD-06 review-before-share** screen → **PRD-10** (pick recipient by public key) → **PRD-05/PRD-11** encrypt-to-recipient → **blob** (clipboard / `.cpblob`). This is the *only* path that produces a share, and it is never automatic. Nothing is transmitted (no server).
...
$ claudepad share ~/.claude/projects/acme/3f2a.jsonl --to steve --tier body+secret
  ...
  📋 wrote share-for-steve.cpblob  ·  copied cp-blob-… to clipboard
...
> /claudepad-share
  ...
  → returns the cp-blob-… (and writes a .cpblob) for the user to send
```
...
- **Output discipline:** human progress to stderr while running, the final `cp-blob` to **stdout** (so `$(claudepad share latest --to steve)` captures just the blob) plus a `.cpblob` file; never print decrypted secret values or keys to logs.
...
`cp-blob-…` (and the `.cpblob` file path) verbatim. Do not print or echo any
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claudepad share \"$(jq -r .transcript_path)\" --yes --quiet || true   # encrypts to the configured default recipient; writes a .cpblob
          }
        ]
      }
    ]
...
- [ ] `claudepad share <file> --to <r>`, `share - --to <r>` (pipe), and `share latest --to <r>` each produce a `cp-blob` (stdout + `.cpblob`) after a client-side scan+encrypt; a network capture shows **zero egress** (FR-10–12; ROADMAP §6).
...
- [ ] `--yes`/non-TTY review keeps detections redacted and never auto-reveals; `--out` controls the `.cpblob` location (FR-15).

...
- **G3.** Output a self-contained **blob** (`cp-blob-…`): auto-copy to clipboard and/or download `.cpblob`. No upload, no link.
...
### 4.3 Large sessions
Above a size threshold the dialog steers to **`.cpblob` file** download/upload instead of clipboard (Q-16); small ones default to clipboard with auto-copy.
...
- **FR-6.** Output MUST **auto-copy** to clipboard on success and offer **Download `.cpblob`**; for sessions above the size threshold the UI MUST steer to file output (Q-16).
...
### Receive
- **FR-9.** The receive action MUST require an unlocked identity and accept a `cp-blob-…` string (or `.cpblob` file).
...
- [ ] Blob conforms to §7, contains no plaintext, serializes as `cp-blob-…`, auto-copies, and downloads as `.cpblob` (FR-5, FR-6).
...
- **Q-16 (large blobs):** clipboard size threshold to auto-switch to `.cpblob` file; consider chunking/compression (PRD-05 may add deflate-before-encrypt).
...
- **New OQ-B:** include a short human-readable header inside the `.cpblob` file (e.g. "claudepad encrypted share for <fingerprint>") so a finder knows what it is? Lean yes (UX), ensure it leaks nothing.

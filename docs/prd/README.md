...
- [`prd-13-usage-insights.md`](./prd-13-usage-insights.md) - **✅ built 2026-06-26:** the Usage Insights ("Usage Stats") analytics surface (token/cost/time reporting over the connected vault, client-side). Shipped in `apps/client/src/usage/**` + `pages/UsageInsights.tsx`, route `#/usage`. See decision **D-91** for as-built notes; the PRD is retained (not deleted) with as-built status.
- [`prd-07-backend.md`](./prd-07-backend.md) - the one v1 PRD that is **not** built: the optional store/backend addon, kept as a blueprint for the vNext registry reference implementation.

...
export const AskUserQuestionTurn = React.memo(function AskUserQuestionTurn({
  event,
  result,
  anchorId,
  highlighted,
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-label uppercase tracking-[0.02em] text-muted">
        <CircleHelp className="size-3.5 shrink-0 text-accent" />
        <span>Question</span>
      </div>
      {questions.map((q, i) => (
        <div key={i} className="space-y-1.5">
          {q.header && <span className="text-label text-muted">{q.header}</span>}
          <p className="text-body-sm font-medium text-text">{q.question}</p>
          <ul className="flex flex-wrap gap-1.5">
            {q.options.map((o, j) => (
              <li key={j}>
                <Badge variant="neutral" title={o.description}>
                  {o.label}
                </Badge>
              </li>
            ))}
